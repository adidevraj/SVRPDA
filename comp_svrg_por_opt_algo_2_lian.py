...
def comp_svrg_por_opt_algo_2(r_matrix, theta, s_size, n, d, m, iters, storage_freq, obj_theta_star, A, prob_str):

    print('Algo 2 Lian et all')
    ...
    for k in range(1, iters):
        ...
        j_k_1 = np.random.randint(n)
        r_j_k_1_transpose = (r_matrix[j_k_1, :]).reshape(1, d)

        G_prime_k = np.zeros((d+1, d))
        G_prime_k[0:d, 0:d] = np.eye(d)
        G_prime_k[d, :] = r_j_k_1_transpose

        G_til_prime_k = G_prime_k
        ...
        f_hat_prime_k = np.matmul(G_prime_k.transpose(), Grad_F_i_k_G_hat_k) - np.matmul(
            G_til_prime_k.transpose(), Grad_F_i_k_G_til) + f_til_prime
    ...
    save_str = './result/comp_svrg_por_opt_algo_2_lian_' + prob_str
