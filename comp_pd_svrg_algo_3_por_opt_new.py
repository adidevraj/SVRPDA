...
def svrpda_2(r_matrix, theta, v, ss_p, ss_d, n, d, m, iters, obj_theta_star, storage_freq, prob_str):
    ...
    for k in range(1, iters):
        ...
        i_k_2 = np.random.randint(0, n)
        j_k_2 = np.random.randint(0, n)

        r_i_k_2 = (r_matrix[i_k_2, :]).reshape(d, 1)
        r_j_k_2 = (r_matrix[j_k_2, :]).reshape(d, 1)

        computations_approx += 2 * d

        if flag == 0:
            grad_v_i_k = np.matmul((r_i_k - r_j_k).transpose(), theta)
            v[i_k] = (1 / (1 + 0.5 * ss_d)) * (v[i_k] + ss_d * grad_v_i_k)
            grad_theta = - r_i_k + (r_i_k - r_j_k)*v[i_k, 0]
            theta = theta - ss_p * grad_theta
        else:
            v_prev_i_k = v[i_k, 0].copy()
            var_red_grad_v_i_k = np.matmul((r_i_k - r_j_k).transpose(), theta - theta_til) + batch_gd_v[i_k]
            v[i_k] = (1 / (1 + 0.5 * ss_d)) * (v_prev_i_k + ss_d * var_red_grad_v_i_k)

            u_k += (v[i_k] - v_prev_i_k) * (r_i_k - r_j_k_2)/n
        ...
        if flag == 0:
            ...
            no_oracle_calls += 3
            # Accessing the stored value (r_i_k - r_j_k_2) counts as one oracle call.
        ...
        if np.remainder(k, m) == 0:
            ...
            no_oracle_calls += n
            u_k = batch_gd_theta_t_2.copy()
            # u_k_2 = batch_gd_theta_t_2.copy()
    ...
    save_str = './result/comp_pd_svrg_algo_3_por_opt_' + prob_str
